...
export async function sendInquiryNotificationEmail(payload: InquiryEmailPayload): Promise<{
  ...
  try {
    ...
    if (!apiKey) {
      console.log(
        `[EMAIL SERVICE] Notification skipped (No RESEND_API_KEY/BREVO_API_KEY set). Inquiry for ${payload.email} saved to Firestore.`
      );
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      if (res.ok) {
        console.log(`[EMAIL SERVICE] Resend notification delivered successfully to ${adminEmail}`);
        return { success: true };
      }
    }
